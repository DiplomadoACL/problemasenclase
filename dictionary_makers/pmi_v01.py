...
for bigrama in dic_bigramas:
    ...
    P_palabra1_seguida_palabra2=float(dic_bigramas[bigrama])/(numero_palabras_corpus-1)
    pmi=math.log(P_palabra1_seguida_palabra2/(P_palabra1*P_palabra2))
    dic_pmi[bigrama]=pmi

lista_bigramas_ordenados=dic_pmi.keys()
lista_bigramas_ordenados.sort(key=lambda x:-dic_pmi[x])
for bigrama in lista_bigramas_ordenados[:20]: 
    print bigrama, dic_pmi[bigrama]
